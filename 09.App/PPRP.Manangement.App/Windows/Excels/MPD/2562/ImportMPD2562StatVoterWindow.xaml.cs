...
{
    /// <summary>
    /// Interaction logic for ImportMPD2562StatVoterWindow.xaml
    /// </summary>
    public partial class ImportMPD2562StatVoterWindow : Window
    {
        #region Constructor
        ...
        /// Constructor.
        /// </summary>
        public ImportMPD2562StatVoterWindow()
        {
            InitializeComponent();
        }
        ...
        private void Model_SheetItemChanges(object sender, ExcelWorksheetArgs evt)
        {
            ...
            {
                var sheet = evt.Sheet;
                lvMapPreview.Setup<MPDStatVoter>(sheet);
            }
        }
        ...
        private void ChooseExcelFile()
        {
            if (model.Open())
            {
                wsMap.Setup<MPDStatVoter>(model);
            }
        ...
        private bool Imports()
        {
            ...
            {
                var obj = item as MPDStatVoter;
                if (null != obj)
                {
                    obj.ThaiYear = year;
                    var ret = MPDStatVoter.Import(obj);
                    if (ret.HasError)
                    {
                        // get debug string.
